...
export const data = new SlashCommandBuilder()
	.setName('skip')
	.setDescription('Assim que eu utilizar VENTUS PULUS essa música irá embora.')
	.addIntegerOption(option =>
		option.setName('para')
			.setDescription('Escolha para que posição da queue você deseja pular. Se necessário utilize /queue')
			.setMinValue(1)
			.setRequired(false));
...
export async function execute(interaction) {
	...
	try {
		...
		if (option) {
			if (isNaN(option)) return interaction.reply('| Por favor, coloque um número valido!');
			await queue.jump(option - 1);
			return interaction.reply({ embeds: [{
				'color': 16777215,
				'author': {
					'name': `A música foi pulada para posição ${option}.`,
					'icon_url': 'https://media.discordapp.net/attachments/1100223066673520663/1103760762469634088/minecraft_1.gif',
				},
			}], ephemeral: false });
		}
		...
		return interaction.reply({ embeds: [{
			...
			'author': {
				...
				'icon_url': 'https://media.tenor.com/zx_m_fhcxW0AAAAi/noot-noot-pingu.gif',
			},
		}], ephemeral: false });
	} catch (err) {
		console.log(err);
