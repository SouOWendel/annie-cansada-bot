...
export const data = new SlashCommandBuilder()
	.setName('queue')
	.setDescription('Você passa a sua playlist para Annie, assim ela faz uma leitura rápida diretamente no chat.');
...
export async function execute(interaction) {
	...
	try {
		...
		embed.setColor('Purple').setDescription(`${queue.songs.map(
			(song, id) => `\n**${id + 1}.** ${ (song.name.length <= 60) ? `[${song.name}]` : `[${song.name.slice(0, 60).concat('...')}]` }(${song.url}) — \`${song.formattedDuration}\``,
		)}`);
		return interaction.reply({ embeds: [embed], ephemeral: true });
	} catch (err) {
		console.log(err);
